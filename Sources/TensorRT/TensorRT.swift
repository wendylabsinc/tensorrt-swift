...
/// Swift-first API scaffolding for working with NVIDIA TensorRT on Linux.
///
...
public struct EngineBuildOptions: Sendable {
    ...
    /// Fixed input shapes used to synthesize a default optimization profile when `profiles` is empty.
    /// Ignored when explicit profiles are provided.
    ///
    /// Provide concrete shapes for each dynamic input binding. Each hint is converted into a
    /// min/opt/max range where all three shapes are identical.
    ///
    /// Example:
    /// ```swift
    /// let options = EngineBuildOptions(
    ...
    /// )
    /// ```
    public var shapeHints: [String: TensorShape]
...
public struct TensorRTRuntime: Sendable {
    ...
    ///   - options: Precision, workspace, optimization profiles, or shape hints for dynamic inputs.
    public func buildEngine(
        onnxURL: URL,
        options: EngineBuildOptions = EngineBuildOptions()
    ) throws -> Engine {
...
public struct Engine: Sendable {
    ...
    /// - Throws: `TensorRTError.invalidBinding` if the engine has no serialized data.
    public func save(to url: URL) throws {
        guard let data = serialized else {
            throw TensorRTError.invalidBinding("Engine has no serialized plan data to save.")
        }
    ...
    public static func load(
        ...
        return try TensorRTRuntime().deserializeEngine(from: data, configuration: configuration)
    }
}
...
public struct DefaultTensorRTNativeInterface: TensorRTNativeInterface {
    ...
    public func buildEngine(onnxURL: URL, options: EngineBuildOptions) throws -> Engine {
        ...
        func validatedShapeHint(_ shape: TensorShape, name: String) throws -> TensorShape {
            guard shape.rank > 0 else {
                throw TensorRTError.invalidShapeRange("Shape hint for \(name) must have rank > 0.")
            }
            guard !shape.isDynamic else {
                throw TensorRTError.invalidShapeRange("Shape hint for \(name) must use only positive dimensions; received \(shape).")
            }
            return shape
        }

        func resolvedProfiles() throws -> [OptimizationProfile] {
            if !options.profiles.isEmpty {
                return options.profiles
            }
            guard !options.shapeHints.isEmpty else {
                return []
            }

            var ranges: [String: OptimizationProfile.BindingShapeRange] = [:]
            ranges.reserveCapacity(options.shapeHints.count)
            for (name, shape) in options.shapeHints {
                let hint = try validatedShapeHint(shape, name: name)
                ranges[name] = .init(min: hint, optimal: hint, max: hint)
            }

            return [
                OptimizationProfile(name: "shape-hints", axes: [:], bindingRanges: ranges),
            ]
        }

        let profiles = try resolvedProfiles()
        ...
        func flattenProfileRanges() -> (names: [String], ranges: [trt_profile_binding_range], profileCount: Int32) {
            ...
            entries.reserveCapacity(profiles.reduce(0) { $0 + $1.bindingRanges.count })
            ...
            return (names, entries, Int32(profiles.count))
        }
        ...
        guard status == 0, let rawPtr, size > 0 else {
            let hint: String
            if profiles.isEmpty && options.shapeHints.isEmpty {
                hint = " If the model uses dynamic shapes, provide EngineBuildOptions.shapeHints or profiles."
            } else {
                hint = ""
            }
            throw TensorRTError.runtimeUnavailable("Failed to build engine from ONNX at \(onnxURL.path) (status \(status)).\(hint)")
        }
